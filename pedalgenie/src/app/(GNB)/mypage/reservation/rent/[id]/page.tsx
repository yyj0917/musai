...
export default function RentDetailPage({ params }: { params: { id: number } }) {
  ...
  if (!rentProductDetail) return null;
        ...
        <p className={`pl-4 text-body1
                ${
                  rentProductDetail?.rentStatus === '픽업예정' || rentProductDetail?.rentStatus === '사용중'
                    ? 'text-green'
                    : rentProductDetail?.rentStatus === '주문확인중' || rentProductDetail?.rentStatus === '반납완료'
                      ? 'text-grey150'
                      : rentProductDetail?.rentStatus === '취소완료'
                        ? 'text-red'
                        : ''
                }`}>
          ...
          {rentInfoBox.map((info, index) => (
            <div
              ...
              <div className="w-full flex justify-between">
                ...
                {(rentProductDetail?.rentStatus === '사용중' || rentProductDetail?.rentStatus === '반납완료') &&
                  index === 3 && <p className="text-body2 text-red">픽업완료</p>}
